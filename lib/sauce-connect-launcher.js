...
function getArchiveName(versionDetails) {
  if (!versionDetails.download_url) {
    throw new Error("Archive name can only be determined from download URL");
  }

  return versionDetails.download_url.split("/").pop();
}

function getScBin(versionDetails) {
  // Sauce Connect 5 archives don't have a sub-folder, so just return the binary name
  // That means that currently using a newer version doesn't download a new binary, but that might be fine for CI usage
  var exe = process.platform === "win32" ? ".exe" : "";
  return path.normalize(scDir + "/"  + "/sc" + exe);
}
...
function fetchArchive(versionDetails, archiveName, archivefile, callback) {
  if (!fs.existsSync(scDir)) {
    fs.mkdirSync(scDir);
  }

  const downloadUrl = versionDetails.download_url;
  const path = downloadUrl.split("/").slice(3).join("/");
...
function fetchAndCheckArchive(versionDetails, archiveName, archivefile, checksum, callback) {
  return async.waterfall([
    async.apply(fetchArchive, versionDetails, archiveName, archivefile),
...
function fetchAndUnpackArchive(versionDetails, options, callback) {
  var bin = getScBin(versionDetails);
  if (exists(bin)) {
    return callback(null, bin);
  }

  var archiveName = getArchiveName(versionDetails);
  ...
  async.waterfall([
    async.apply(fetchAndCheckArchive, versionDetails, archiveName, archivefile, versionDetails.checksum),
...
function readVersionsFile(versionsfile, cb) {
  ...
  return cb(null, {
    ...
    download_url: versions[scPlatform()]["download_url"],
  });
}
...
function getVersion(options, cb) {
  ...
  // Use local copy of versions.json for now, which contains download links for
  // Sauce Connect 5 in the same format that the old versions.json file did
  fs.cpSync(path.normalize(__dirname + "/local_versions.json"), versionsfile);
